...
{	
	...
	void SetVisible( GameObject obj, bool visible, bool isHand)
	{
        if (isHand)
        {
            foreach (Renderer child in obj.GetComponentsInChildren<Renderer>())
            {
                if (child.tag == "FingerTip" && IsEnablingFingerRenderers)
                    child.enabled = visible && m_DisplayHands;
                else if(child.tag!="FingerTip" && IsEnablingFingerChildrenRenderers)
                    child.enabled = visible && m_DisplayHands;
                else
                    child.enabled = false;
            }
        ...
        else
        {
            foreach (Renderer child in obj.GetComponentsInChildren<Renderer>())
            {
                if (child.gameObject.tag!="FingerTip" && IsEnablingFingerChildrenRenderers)
                    child.enabled = visible && m_DisplayHands;
                else if(child.gameObject.tag=="FingerTip" && IsEnablingFingerRenderers)
                    child.enabled = visible && m_DisplayHands;
                else
                    child.enabled = false;
            }
		...
		foreach( Renderer component in obj.GetComponents<Renderer>() )
        {
            if ((isHand && IsEnablingPalmRenderers) || (!isHand && ( (IsEnablingFingerRenderers && obj.tag == "FingerTip") ||  IsEnablingFingerChildrenRenderers)))
		        component.enabled = visible && m_DisplayHands;
            else
                component.enabled = false;            
		}
		...
		foreach( Renderer child in obj.GetComponentsInChildren<Renderer>() ) 
        {
            if (child.transform.parent.tag == "FingerTip" && IsEnablingFingerChildrenRenderers)
                child.enabled = visible && m_DisplayHands;
            else           
                child.enabled = false;
		}*/
	...
	void updatePointable( Leap.Pointable pointable, GameObject fingerObject )
	{
        ...
        if((pointable.Hand.PalmPosition.ToUnityTranslated()-pointable.TipPosition.ToUnityTranslated()).magnitude < Magnitude)
            isFarEnough = false;


		SetVisible(fingerObject, pointable.IsValid && isFarEnough,false);
		SetCollidable(fingerObject, pointable.IsValid && isFarEnough);
		
		if ( pointable.IsValid && isFarEnough)
		{
			Vector3 localFingerPos = pointable.TipPosition.ToUnityTranslated();
			
			float fingerRadius = fingerObject.GetComponent<SphereCollider>().radius - 0.25f;
			RaycastHit target;
			...
			fingerObject.GetComponent<Rigidbody>().velocity = pointable.TipVelocity.ToUnityScaled();
	...
	void updatePalm( Leap.Hand leapHand, GameObject palmObject )
	{
		...
		SetVisible(palmObject, leapHand.IsValid,true);
